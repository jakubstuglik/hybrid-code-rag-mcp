...
| T11 | `PrepareDataSet` | Method implementation (`defProc`) in one of the Pascal units. | `node_type` in {`defProc`, `defProc_split`, `method_group`}, text content contains `PrepareDataSet`, position <= 2 | Easy | Sparse/BM25: method name as single token |
