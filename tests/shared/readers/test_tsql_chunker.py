...
class TestTSqlChunk:
    ...
    def test_required_fields(self):
        ...
        chunk = TSqlChunk(text="SELECT 1", node_type="sql_batch", start_line=1, end_line=1)
...
class TestIsPreambleBatch:
    ...
    def test_multiple_preamble(self):
        assert _is_preamble_batch(["SET QUOTED_IDENTIFIER ON", "", "SET ANSI_NULLS ON"]) is True
...
class TestClassifyDdlBatch:
    ...
    def test_create_index(self):
        assert _classify_ddl_batch(["CREATE NONCLUSTERED INDEX IX ON dbo.T(x)"]) == "create_index"
    def test_exec(self):
...
class TestDetectObject:
    ...
    def test_procedure_extracts_params(self):
        lines = ["CREATE PROCEDURE dbo.MyProc(", "  @x INT,", "  @y NVARCHAR(50)", ")", "AS", "BEGIN"]
...
class TestExtractParameters:
    ...
    def test_multi_line_params(self):
        lines = ["CREATE PROCEDURE [dbo].[Bar](",
                 "  @a NVARCHAR(10),", "  @b INT = 0", ")", "AS"]
        result = _extract_parameters(lines)
        assert "@a" in result
        assert "@b" in result
    ...
    def test_stops_at_as(self):
        lines = ["CREATE PROCEDURE dbo.Foo(", "  @x INT", ")", "AS", "BEGIN", "  DECLARE @y INT"]
        result = _extract_parameters(lines)
        assert "@x" in result
        assert "@y" not in result
    def test_comments_skipped(self):
        lines = ["CREATE PROCEDURE dbo.Foo(", "  @a INT,", "  -- comment", "  @b VARCHAR(20)", ")", "AS"]
        result = _extract_parameters(lines)
        assert "@a" in result
        assert "@b" in result
    def test_no_create_returns_none(self):
...
class TestFindDeclarationsEnd:
    ...
    def test_body_start_offset(self):
        lines = ["CREATE PROCEDURE dbo.Foo", "AS", "BEGIN", "DECLARE @x INT", "SET @x = 1", "SELECT @x"]
...
class TestFindSectionBoundaries:
    ...
    def test_consecutive_dashes_single_boundary(self):
        lines = ["SELECT 1", "-"*25, "-"*22+"BILETY"+"-"*3, "-"*25, "SELECT 2"]
...
class TestFindDynamicSqlRanges:
    ...
    def test_simple_dynamic_sql(self):
        lines = ["SET @Sql = 'SELECT 1'", "SET @Sql = @Sql + ' FROM dbo.T'", "EXEC(@Sql)"]
...
class TestGroupSmallDdlBatches:
    ...
    def test_no_ddl_unchanged(self):
        chunks = [TSqlChunk(text="X"*200, node_type="procedure_body", start_line=1, end_line=10)]
        result = _group_small_ddl_batches(chunks)
        assert len(result) == 1
    def test_small_ddl_grouped(self):
        chunks = [
            TSqlChunk(text="ALTER TABLE T1 ADD x INT", node_type="alter_table", start_line=1, end_line=2),
            TSqlChunk(text="ALTER TABLE T1 ADD y INT", node_type="alter_table", start_line=3, end_line=4),
            TSqlChunk(text="ALTER TABLE T1 ADD z INT", node_type="alter_table", start_line=5, end_line=6),
        ]
        result = _group_small_ddl_batches(chunks)
        assert len(result) == 1
    ...
    def test_grouped_same_type_suffix(self):
        chunks = [
            TSqlChunk(text="ALTER TABLE T ADD a", node_type="alter_table", start_line=1, end_line=1),
            TSqlChunk(text="ALTER TABLE T ADD b", node_type="alter_table", start_line=2, end_line=2),
        ]
        result = _group_small_ddl_batches(chunks)
    ...
    def test_mixed_types_ddl_group(self):
        chunks = [
            TSqlChunk(text="ALTER TABLE T ADD a", node_type="alter_table", start_line=1, end_line=1),
            TSqlChunk(text="CREATE INDEX IX ON T(a)", node_type="create_index", start_line=2, end_line=2),
        ]
        result = _group_small_ddl_batches(chunks)
        assert result[0].node_type == "ddl_group"
    def test_single_ddl_no_group_suffix(self):
        chunks = [TSqlChunk(text="ALTER TABLE T ADD a", node_type="alter_table", start_line=1, end_line=1)]
        result = _group_small_ddl_batches(chunks)
        assert result[0].node_type == "alter_table"
    def test_group_preserves_lines(self):
        chunks = [
            TSqlChunk(text="ALTER TABLE T ADD a", node_type="alter_table", start_line=10, end_line=12),
            TSqlChunk(text="ALTER TABLE T ADD b", node_type="alter_table", start_line=15, end_line=18),
        ]
        result = _group_small_ddl_batches(chunks)
...
class TestForceSplitOversized:
    ...
    def test_normal_chunk_unchanged(self):
        chunk = TSqlChunk(text="SELECT 1\n" * 10, node_type="procedure_body", start_line=1, end_line=10)
        result = _force_split_oversized([chunk])
        assert len(result) == 1
    def test_oversized_chunk_split(self):
        big_text = "\n".join([f"SELECT column_{i}" for i in range(2000)])
        chunk = TSqlChunk(text=big_text, node_type="procedure_body", start_line=1, end_line=2000,
                          object_name="dbo.BigProc", object_type="PROCEDURE")
        result = _force_split_oversized([chunk])
        assert len(result) > 1
        for sub in result:
...
class TestIntegrationRealFiles:
    ...
    def company_branches_content(self):
        ...
        if not path.exists(): pytest.skip(f"Not found: {path}")
    ...
    def test_company_branches_object_type(self, company_branches_content):
        assert "PROCEDURE" in {c.object_type for c in chunk_tsql(company_branches_content) if c.object_type}
    def test_company_branches_has_header(self, company_branches_content):
        assert "procedure_header" in _chunk_node_types(chunk_tsql(company_branches_content))
    def test_company_branches_has_parameter(self, company_branches_content):
        params = [c.parameters for c in chunk_tsql(company_branches_content) if c.parameters]
    ...
    def test_sls_ticket_ddl_grouped(self, sls_ticket_content):
        alter_chunks = [c for c in chunk_tsql(sls_ticket_content) if c.node_type == "alter_table"]
    ...
    def test_analysis_route_params(self, analysis_route_content):
        params = [c.parameters for c in chunk_tsql(analysis_route_content) if c.parameters]
        assert len(params) > 0
    ...
    def test_fare_price_is_function(self, fare_price_content):
        assert "FUNCTION" in {c.object_type for c in chunk_tsql(fare_price_content) if c.object_type}
    ...
    def test_relief_payments_object_type(self, relief_payments_content):
        assert "PROCEDURE" in {c.object_type for c in chunk_tsql(relief_payments_content) if c.object_type}


    # --- Cross-file tests ---
    def test_all_real_files_produce_chunks(self):
        sql_files = list(_TEST_SOURCES.glob("*.sql"))
        if not sql_files: pytest.skip("No SQL test source files found")
